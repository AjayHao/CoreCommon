...
import com.ajayhao.core.enums.RoundMode;
...
public abstract class CoreDateUtils {
     ...
     * @return
     */
    public static boolean isYearMid(Date date, RoundMode mode) {
        return inYear(date, -2, mode);
    }

    private static boolean inYear(Date date, int expect, RoundMode mode) {
        Calendar c = getCalendar(date);
        ...
        switch (expect) {
            ...
            case -2: // 年中；
                ...
                return decimal.setScale(0, mode.getValue()).intValue() == curr;
            default:
                return expect == curr;
        }
    }
     ...
     * @return
     */
    public static boolean isMonthMid(Date date, RoundMode mode) {
        return inMonth(date, -2, mode);
    }

    private static boolean inMonth(Date date, int expect, RoundMode mode) {
        Calendar c = getCalendar(date);
     ...
     * @return
     */
    public static boolean isQuarterMid(Date date, RoundMode mode) {
        return inQuarter(date, -2, mode);
    }

    private static boolean inQuarter(Date date, int expect, RoundMode mode) {
        Calendar c = getCalendar(date);
        ...
        switch (expect) {
            ...
            case -2: // 季中；
                ...
                final int mid = decimal.setScale(0, mode.getValue()).intValue();
...
//============================4.时间加减=====================================    
    ...
    public static final Date addMonths(Date aDate, int amount, boolean isOracle) {
        ...
        if (isOracle && CoreDateUtils.isMonthEnd(aDate)) {
            date = CoreDateUtils.getMonthEnd(date);
        }
        return date;
    }
